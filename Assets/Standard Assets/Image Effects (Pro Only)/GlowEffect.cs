...
// Glow uses the alpha channel as a source of "extra brightness". All builtin Unity shaders output
// baseTexture.alpha * color.alpha, plus specularHighlight * specColor.alpha into that. Usually
// you'd want either to make base textures to have zero alpha; or set the color to have zero alpha
// (by default alpha is 0.5).
...
public class GlowEffect : MonoBehaviour
{
    ...
    /// Blur spread for each iteration. Lower values give better looking blur, but require more
    /// iterations to get large blurs. Value is usually between 0.5 and 1.0.
    ...
    // -------------------------------------------------------- The final composition shader: adds
    // (glow color * glow alpha * amount) to the original image. In the combiner glow amount can be
    // only in 0..1 range; we apply extra amount during the blurring phase.
    ...
    // -------------------------------------------------------- The blur iteration shader. Basically
    // it just takes 4 texture samples and averages them. By applying it repeatedly and spreading
    // out sample locations we get a Gaussian blur approximation. The alpha value in _Color would
    // normally be 0.25 (to average 4 samples), however if we have glow amount larger than 1 then we
    // increase this.
    ...
    // -------------------------------------------------------- The image downsample shaders for
    // each brightness mode. It is in external assets as it's quite complex and uses Cg.
