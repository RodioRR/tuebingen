...
public class ContrastStretchEffect : MonoBehaviour
{
    ...
    /// If our scene is really dark (or really bright), we might not want to stretch its contrast to
    /// the full range. limitMinimum=0, limitMaximum=1 is the same as not applying the effect at
    /// all. limitMinimum=1, limitMaximum=0 is always stretching colors to full range.
    ...
    // To maintain adaptation levels over time, we need two 1x1 render textures and ping-pong
    // between them.
    ...
    // Reduces size of the image by 2x2, while computing maximum/minimum values. By repeatedly
    // applying this shader, we reduce the initial luminance image to 1x1 image with minimum/maximum
    // luminances found.
    ...
    // Adaptation shader - gradually "adapts" minimum/maximum luminances, based on currently adapted
    // 1x1 image and the actual 1x1 image of the current scene.
    ...
    // Final pass - stretches the color values of the original scene, based on currently adpated
    ...
    private void OnRenderImage ( RenderTexture source, RenderTexture destination )
    {
        ...
        // Repeatedly reduce this image in size, computing min/max luminance values In the end we'll
        // have 1x1 image with min/max luminances found.
    ...
    private void CalculateAdaptation ( Texture curTexture )
    {
        ...
        // Adaptation speed is expressed in percents/frame, based on 30FPS. Calculate the adaptation
        // lerp, based on current FPS.
