...
namespace URandom
{
    // Returns a deviate distributed as a gamma distribution of integer of order ia, i.e. the
    // waiting time for the iath event in a Poisson process of unit mean Use _rand as a source of
    // uniform deviates
        ...
        public static float Normalize ( ref NPack.MersenneTwister _rand, int ia )
        {
            ...
            else
            {
                do
                {
                    do
                    {
                        // This four lines generate the tanget of random angle Equivalent to y =
                        // tan( pi * rand())
                        do
                        {
