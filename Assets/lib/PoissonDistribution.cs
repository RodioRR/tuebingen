...
namespace URandom
{
    ...
    {
        // return as a floating point number an integer value that is a random deviate drawn from a
        // Possion Distribution of mean xm using randx as a source of uniform deviates
        public static float Normalize ( ref NPack.MersenneTwister _rand, float xm )
        {
            ...
            {      // Use direct method
                ...
                em = -1;
                t = 1.0f;
                // Instead of adding exponential deviates it is equivalent to multiply unifomr
                // deviates We never actually take the log, we compare the precomupted exponential
                do
                {
                    ++em;
            ...
            {
                // Use REJECTION method xm has changed? 
                if (xm != oldm)
                {
                    oldm = xm;
                    sq = Math.Sqrt(2.0 * xm);
